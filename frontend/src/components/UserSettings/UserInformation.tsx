    ...
    <>
      <Container maxW="full">
        ...
        >
          ...
          <Field
            mt={4}
            label={t("settings.userInformation.geminiApiKey")}
            helperText={t("settings.userInformation.geminiApiKeyHelp")}
          >
            {editMode ? (
              <InputGroup
                endElement={
              ...
              >
                <Input
                  ...
                  size="md"
                  placeholder={t("settings.userInformation.enterGeminiKey")}
                  width="full"
                />
              </InputGroup>
            ) : (
              <Text fontSize="md" py={2} truncate maxW="full" color={!currentUser?.gemini_api_key ? "gray" : "inherit"}>
                {currentUser?.gemini_api_key ? t("settings.userInformation.apiKeyHidden") : t("settings.userInformation.notSet")}
              </Text>
            )}
          </Field>
          {/* Preferred Gemini Model Field */}
          <Field
            mt={4}
            label={t("settings.userInformation.preferredGeminiModel")}
            helperText={t("settings.userInformation.preferredGeminiModelHelp")}
          >
            {editMode ? (
              // Use native select instead of Chakra UI Select
              <Box
                width="full"
                borderRadius="md"
                border="1px solid"
              ...
              >
                <select
                  {...register("preferred_gemini_model", { required: true })}
                  style={{ 
                    width: "100%", 
                    padding: "8px 16px", 
                    borderRadius: "md", 
              ...
              <Text
                fontSize="md"
                py={2}
                color={!currentUser?.preferred_gemini_model ? "gray" : "inherit"}
                truncate
                maxW="full"
              >
                {currentUser?.preferred_gemini_model || t("settings.userInformation.defaultModel")}
              </Text>
            )}
          </Field>
          <Flex mt={4} gap={3}>
